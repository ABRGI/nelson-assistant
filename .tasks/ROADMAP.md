...
| Cluster hints in Sonnet pre-injection (Phase F.2) | ✅ done (2026-04-23) | `src/state/topic-hints.ts` — loads latest `analytics/topics/<date>.json` at boot; per-turn substring match against cluster sample/representative questions (both directions); top-2 by frequency prepended to Sonnet's system prompt between thread state and leaves. SIGHUP from `run-topic-analysis.js --save` live-reloads without a restart. 9 new tests. |
