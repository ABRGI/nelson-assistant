...
async function loadSystemSeed(project: string, tenant: ClientRecord, psqlReadOnlyUrl?: string, knowledgeInjection?: string): Promise<string> {
  const parts: string[] = [
    ...
    `dateMode selection for /api/management/secure/reservations (HARD RULE — getting this wrong silently returns the wrong count):\n- User says "arrivals", "arriving", "check-ins", "who's coming", "reservations for <date>" → \`dateMode=ARRIVAL\`.\n- User says "departures", "check-outs", "leaving" → \`dateMode=DEPARTURE\`.\n- User says "staying", "in-house", "occupancy on", "guests tonight", "staying tonight" → \`dateMode=STAY\`.\n- User says "booked on", "created on", "new bookings", "confirmed on" → \`dateMode=CREATED\`.\n- \`dateMode=EXACT\` means "reservation EXACTLY matches a 1-night range (check_in = startDate AND check_out = endDate)" — almost NEVER what the user wants. Do NOT default to EXACT.\n- When in doubt between ARRIVAL and STAY, use ARRIVAL (most common intent).\n- SANITY CHECK: if your first query returns 0 rows on a date where activity clearly exists, you picked the wrong dateMode. Retry with ARRIVAL → STAY → CREATED before concluding "no reservations". If the report says 54 arrivals and you got 7, you used EXACT — switch to ARRIVAL immediately.\n- Always pair with \`totalCount=true\` so you see the full count rather than a truncated first page. For the first page to match the total, use a narrow single-date window (startDate=endDate).\n- Authoritative cross-check — if the HTTP API answer looks wrong, verify with: \`mcp__nelson__psql\` running \`SELECT state, COUNT(*) FROM nelson.reservation WHERE hotel_id = <id> AND DATE(check_in) = '<date>' GROUP BY state\` (swap to check_out for departures, use overlap for stays). DB is authoritative.\nSee knowledge/nelson/endpoints/reservations.yaml → dateMode_semantics + dateMode_pick_procedure.`,
